...
struct args{
    int connfdpointer;
    struct sockaddr_in clientaddrpointer;
};
...
int main(int argc, char **argv){
    ...
    while (1) {
        ...
        struct args *new_args = malloc(sizeof(struct args));
        new_args->connfdpointer = connfd;
        new_args->clientaddrpointer = clientaddr;

        pthread_create(&tid, NULL, handle_request, (void*)new_args);
        pthread_detach(tid); 
    }
...
void *handle_request(void *arguments) {
    ...
    struct args *new_args = (struct args*)arguments;
    int connfd = new_args->connfdpointer;
    struct sockaddr_in clientaddr = new_args->clientaddrpointer;
